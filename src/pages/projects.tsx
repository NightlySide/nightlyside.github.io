  ...
  return (
    <MainLayout>
      ...
      <div className="w-full bg-secondary">
        <h1 className="py-7 px-16 text-4xl text-primary font-black text-center md:text-left">Mes derniers projets</h1>
        <div className="w-full px-4 md:px-8 xl:px-32 mb-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {showcase_posts.map((post, index) => (
            <ShowcaseCard key={index} post={post.node} basePath="projects" />
          ))}
        </div>
        <div className="w-full bg-primary px-16 py-8">
          <h1 className="pb-7 text-4xl text-primary font-black lg:px-32 text-center md:text-left">Autres projets</h1>
          <div className="flex flex-col space-y-10 lg:px-32">
            {posts.map((project, index) => (
              <ListItem post={project.node} basePath="projects" key={index} />
            ))}
          </div>
        </div>
      </div>
